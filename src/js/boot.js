    ...
    function askPassword() {
        ...
        passwordField.addEventListener("keyup", event => {
            if (passwordField.value == "12345") {
                ...
                lockScreen.remove();
                if (firstBoot) {
                    setTimeout(notify, 1e3, "src/icons/settings.svg", "Settings", "Welcome to WineBOX", "Congratulations You have successfully Booted WineBOX")
                    setTimeout(_ => {
                    ...
                    setTimeout(notify, 10e3, "src/icons/edge.svg", "Microsoft Edge", "All new Browser is here", "Your Edge browser got updated for WineBOX	 \n Please try it out.")
                    startTutorials();
                    setCookie("firstBoot", "0");
                }
                if (getCookie("cookiesAccepted") == "") {
                    notify("src/icons/settings.svg", "Settings", "WineBOX uses Cookies", "To improve user experience this WineBOX uses cookies", "Accept", `setCookie('cookiesAccepted','1');notificationClose('#n${nId}')`);
                }
