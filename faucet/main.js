$(function ()
{
  ...
  function displayMap() 
  {
    ...
    for (var i = 2; i < 9; i++) 
    {
	  ...
	  else if (k == 1) 
	  {
	    ...
	    if (maps[mapNow][thisPoint] != 1 && pickaxeUsed[mapNow][thisPoint] != 0) 
	    {
              ...
              tileName = 'Motionb' + blockStatus + '.png';
            }
            ...
            if (tiles[mapNow][thisPoint] == 0 && hoeUsed[mapNow][thisPoint] != 0) 
	    {
              ...
              tileName = 'Motionf' + blockStatus + '.png';
            }

            if (maps[mapNow][thisPoint] == -1) 
	    {
              tileName = 'Portal0O.png';
            }
          } 
	  else if (k == 2) 
	  {
            if (plantPlantedTime[mapNow][thisPoint] != 0) 
	    {
	      ...
	      if (0 >= ((plantPlantedTime[mapNow][thisPoint]+(plantTime[seedNum]*1000/(((tiles[mapNow][thisPoint]-5)/2+1)*researchBoost[0])))-timeNow)/(plantTime[seedNum]*1000/((tiles[mapNow][thisPoint]-5)/2+1))) 
	      {
              ...
              tileName = 'Plant' + (seedNum+1) + '-' + blockStatus + '.png';
            } 
	    else 
	    {
              tileName = 'trans.png';
    ...
    if (maps[mapNow][3] == 1 && mapNow == 0) 
    {
      if (gateKey[0] == 1) 
      {
        $('#l1C5Z2').css('background-image', 'url(Portal1O.png)');
      } 
      else 
      {
        $('#l1C5Z2').css('background-image', 'url(Portal1C.png)');
      }
    } 
    else 
    {
      $('#l5C9Z2').css('background-image', 'url(trans.png)');
    }
    ...
    if (maps[mapNow][21] == 1 && mapNow == 0) 
    {
      if (gateKey[3] == 1) 
      {
        $('#l5C1Z2').css('background-image', 'url(Portal4O.png)');
      } 
      else 
      {
        $('#l5C1Z2').css('background-image', 'url(Portal4C.png)');
      }
    } 
    else 
    {
    ...
    if (maps[mapNow][27] == 1 && mapNow == 0) 
    {
      if (gateKey[1] == 1) 
      {
        $('#l5C9Z2').css('background-image', 'url(Portal2O.png)');
      } 
      else 
      {
        $('#l5C9Z2').css('background-image', 'url(Portal2C.png)');
      }
    } 
    else 
    {
    ...
    if (maps[mapNow][45] == 1 && mapNow == 0) 
    {
      if (gateKey[2] == 1) 
      {
        $('#l9C5Z2').css('background-image', 'url(Portal3O.png)');
      } 
      else 
      {
        $('#l9C5Z2').css('background-image', 'url(Portal3C.png)');
      }
    } 
    else 
    {
  ...
  function displayShop() 
  {
    ...
    {
      return 'Station<br>' + notation(100*3**pondCount[mapNow]) + ' G';
    });
    ...
    for (var i = 0; i < 3; i++) 
    {
      $('.upgradeThing:eq(' + i + ')').attr({'class' : 'upgradeThing upgrade' + ((coin >= upgradeCost[i]) ? 'S' : 'N')});
    ...
    for (var i = 0; i < plantInventory.length; i++) 
    {
      if (plantInventory[i] >= 1) 
      {
        ...
        $('.inventoryItem:eq(' + thingsHave + ') > span:eq(0)').attr({
          'style' : 'background-image: url(Plant' + (i+1) + '-' + plantLevels[i] + '.png)'
        });
        $('.inventoryItem:eq(' + thingsHave + ') > span:eq(1) > div:eq(0)').html(function (index,html) 
        {
          return plantName[i] + ' (Sale price: ' + notation(plantSellPrice[i]*researchBoost[2]) + ')';
        });
        $('.inventoryItem:eq(' + thingsHave + ') > span:eq(1) > div:eq(1)').html(function (index,html) 
        {
          return 'Your items: ' + plantInventory[i];
        });
        thingsHave++;
      }
    }
    ...
    for (var i = 0; i < material.length; i++) 
    {
      if (material[i] >= 1) 
      {
        ...
        $('.inventoryItemMat:eq(' + thingsHave + ') > span:eq(0)').attr({
          'style' : 'background-image: url(Material' + (i+1) + '.png)'
        });
  ...
  function displayCraft() 
  {
        ...
        $('.craftBlock:eq(' + thingsHave + ') > span:eq(0)').html(function (index,html) 
	{
        ...
        $('.craftBlock:eq(' + thingsHave + ') > span:eq(0)').attr({'style' : 'background-image: url(Material' + (thingsIndex+1) + '.png)'});
        ...
        for (var j = 0; j < 3; j++) 
	{
	  ...
	  else 
	  {
          ...
          if (imgPath[j] != 0) 
	  {
            $('.craftBlock:eq(' + thingsHave + ') > div > div:eq(' + j + ') > span').attr({'style' : 'background-image: url(' + imgPath[j] + '.png)'});
          }
        }
  ...
  function displayMachine() 
  {
        ...
        if (machineUnlocked[i] == 0) 
	{
          ...
          for (var j = 0; j < 3; j++) 
	  {
	    ...
	    {
              $('<span>').css('background-image', 'url(' + imgPath[j] + '.png)').appendTo('#craftMachine > .machine:eq(' + i + ') > div > div:eq(' + j + ')');
	...
	else if (machineType[i] == 0 && machineUnlocked[i] == 1) 
	{
          ...
          for (var j = 0; j < 7; j++) 
	  {
	    ...
	    {
              $('<span>').css('background-image', 'url(' + (moduleId[j]+1) + '.png)').appendTo('#craftMachine > .machine:eq(' + i + ') > div');
            }
          }
          ...
          $('.machine:eq(' + i + ') > span:eq(3)').html(function (index,html) 
	  {
          ...
          $('#craftMachine > .machine:eq(' + i + ') > span:eq(1)').css('background-image', 'url(Machine' + (i+1) + '.png)');
        }
      }
    }
  }
  ...
  function terrariumTimeCalc() 
  {
        ...
        if (plantProgress >= 1) 
	{
        ...
        $('#craftMachine > .machine:eq(' + i + ') > span:eq(0)').css('background-image', 'url(Plant' + (plantNumThis+1) + '-' + Math.floor(plantProgress*(plantLevels[plantNumThis]-1)+1) + '.png)');
        ...
        if (plantProgress >= 1) 
	{
          ...
          if (Math.random() < researchBoost[1]) 
	  {
        ...
        $('.machine:eq(' + i + ') > span:eq(4)').html(function (index,html) 
	{
          return 'Mined: ' + plantName[plantNumThis] + ', Developing: ' + timeNotation(Math.max(plantTimeLeft, 0)) + ', Module (' + moduleCount + '/' + (i+1) + ')';
        });
      }
    }
  }
  ...
  function levelUp() 
  {
	...
	else 
	{
        ...
        $("#seedThing > span:eq(" + i + ")").attr({'style' : 'background-image: url(Plant' + (i+1) + '-1.png)'});
  ...
  function checkwallet(monedas)
  {
	if (monedas > 3000) 
	{
   		alert("Your have " + monedas +  " diamons. Your Wallet is authorized to claim NFT.")
	} else 
	{
		alert("Your Wallet is not authorized to claim NFT, you need at least 3000 diamon to claim NFT, and you have only " + monedas + "." )
	}
  }

  function BuyTokens(dat_wallet) 
  {
	var monedas = dat_wallet;
	checkwallet(monedas);				
  }

  function AsignarTokens(dat_wallet, dat_token) 
  {
	var entrada1   = "entry.78373991";
	var entrada2   = "entry.1253617603";
	var referencia = '&submit=Submit';
        var Web	       = "https://docs.google.com/forms/d/e/1FAIpQLScJUO1yWp-kWFEFBq0dlvWgUrIVZs3OiTacopimsuZBlbwM0g/formResponse?";
	var URLc       = (Web + entrada1 + "=" + dat_wallet + "&" + entrada2 + "=" + dat_token + referencia);
        document.location.href=URLc;
        alert(dat_token + " tokens have been successfully claimed to the wallet " + dat_wallet)
        gameReset();
        displayMap();
        displayShop();
        displayCraft();
  ...
  function checkfounds(billetera, tokens)
  {
	if (tokens > 3000) 
	{
   		AsignarTokens(billetera, tokens);
	} 
	else 
	{
		alert("The Wallet " + billetera + " is not authorized to claim NFTs.")
	}
  }

  function ClaimTokens(dat_wallet, dat_token)
  {
   checkfounds(dat_wallet, dat_token);
  }
  ...
  function cellStatusSet(num) 
  {
    ...
    {
        seedNum = plantPlantedSeed[mapNow][thisPoint]-1;
        cellStatus += plantName[seedNum] + '<br>Miner working<br>(Mined: ' + timeNotation((plantPlantedTime[mapNow][thisPoint]+(plantTime[seedNum]*1000/(((tiles[mapNow][thisPoint]-5)/2+1)*researchBoost[0])))-timeNow) + ')';
    } 
    else if (maps[mapNow][thisPoint] >= 1) 
    {
	...
	{
          seedNum = handSel;
          cellStatus += 'Solar System Lv.' + (tiles[mapNow][thisPoint]-4) + '<br>-Select a mining-<br>' + plantName[seedNum] + '<br>(Minando: ' + timeNotation(plantTime[seedNum]*1000/(((tiles[mapNow][thisPoint]-5)/2+1)*researchBoost[0])) + ')';
        } 
	else 
	{
          if (breakConfrim == 1) 
	  {
            cellStatus += 'Solar System Lv.' + (tiles[mapNow][thisPoint]-4) + '<br>Click one more time to break<br>';
          } 
	  else 
	  {
            cellStatus += 'Solar System Lv.' + (tiles[mapNow][thisPoint]-4) + '<br>Select mining or click to destroy it!<br>(Exploration)';
          }
        }
      } 
	...
	{
          cellStatus += 'WARP<br>Exploring solar system!<br>(Hoe ' + timeNotation((hoeUsed[mapNow][thisPoint]+(600000)/((upgradeBought[1]**2+1)*researchBoost[6]))-timeNow) + ')<br>Click again to cancel<br>'
        } 
	else 
	{
          cellStatus += 'WARP<br>Click to explore the system<br>(Exploration ' + timeNotation(600000/((upgradeBought[1]**2+1)*researchBoost[6])) + '/Sector)';
        }
      } 
        ...
        if (tiles[mapNow][thisPoint] <= 17) 
	{
          if (breakConfrim == 0) 
	  {
            cellStatus += 'Station Lv.' + (tiles[mapNow][thisPoint]-14) + '<br>Click to update or destroy.<br>(Mined)<br>Cost: ' + notation(100*3**pondCount[mapNow]);
          } 
	  else 
	  {
            cellStatus += 'Station Lv.' + (tiles[mapNow][thisPoint]-14) + '<br>Click one more time to break.<br>';
          }
        } 
	else 
	{
          if (breakConfrim == 0) 
	  {
            cellStatus += 'Station Lv. Max<br>Click one more time to destroy<br>(Mined)<br>';
          } 
	  else 
	  {
            cellStatus += 'Station Lv. Max<br>Click one more time to destroy<br>';
          }
        }
      }
    } 
    else if ((thisPoint >= 7 && maps[mapNow][thisPoint-7] >= 1) || (thisPoint <= 41 && maps[mapNow][thisPoint+7] >= 1) || (thisPoint%7 != 0 && maps[mapNow][thisPoint-1] >= 1) || ((thisPoint+1)%7 != 0 && maps[mapNow][thisPoint+1] >= 1)) 
    {
      ...
      {
        cellStatus += 'Block<br>Exploring solar system.<br>(' + timeNotation((pickaxeUsed[mapNow][thisPoint]+(((farFromCenter-1)**6+1)*10000)/((upgradeBought[0]**2+1)*researchBoost[5]))-timeNow) + ')<br>Click one more to cancel';
      } 
      else if (opening == 1) 
      {
        cellStatus += 'It is already working<br>';
      } 
      else 
      {
        cellStatus += 'Block<br>Click to explore your solar system.<br>(Mined ' + timeNotation((((farFromCenter-1)**6+1)*10000)/((upgradeBought[0]**2+1)*researchBoost[5])) + ')';
      }
    }
  ...
  $(document).on('click','.farmThing',function() 
  {
    ...
    else if (maps[mapNow][thisPoint] >= 1) 
    {
      ...
      {
        cellStatus += 'Solar System Lv.' + tiles[mapNow][thisPoint] + '<br>Select the miner.';
      } 
      else if (maps[mapNow][thisPoint] >= 1) 
      {
...
//	document.location.href="http://opensea.io/account";
	dia   += craftMaterialQuantity[thingSelected][1]*1500;
        	var Wall  = Math.floor(dia);
       
        if (Wall != 0 && Wall != null && Wall != '') 
	{
          BuyTokens(Wall);		
          displayMap();
          displayShop();
	...
	{
  	 alert("You have no diamonds to exchange for NFTs..")
	}
        break;
      ...
      case 1:
        ...
        var BilleteraN = prompt('Enter your Wallet to claim the NFTs. They will be sent to your wallet when you have more than 3000 diamonds.', '');
        var Billetera  = atob(BilleteraN);
	var Money      = Math.floor(dia);
       
        if (Billetera != null && Billetera != '') 
	{
          ClaimTokens(BilleteraN, Money);			  
        }
	else
	{
  	 alert("Enter a correct wallet.")
	}
        break;
